...
namespace Analyzer1
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(ThreadStaticFieldsCodeFixProvider)), Shared]
    public class ThreadStaticFieldsCodeFixProvider : CodeFixProvider
    {
        public override ImmutableArray<string> FixableDiagnosticIds => ImmutableArray.Create(ThreadStaticFieldNamingAnalyzer.DiagnosticId);
        ...
        private static FieldDeclarationSyntax ReplaceVariableName(VariableDeclaratorSyntax variableDeclarator, FieldDeclarationSyntax fieldDeclaration)
        {
            ...
            var newVariables = variables.Select(v =>
            {
                if (v.Identifier.Text == variableDeclarator.Identifier.Text)
                    return v.WithIdentifier(SyntaxFactory.Identifier("t_" + v.Identifier.Text));
                return v;
            });
